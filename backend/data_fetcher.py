...
class DataFetcher:
    ...
    def _increment_fetch_count(self, pool_id: str, db=None):
        ...
        try:
            ...
            if latest_snapshot:
                features = (
                    dict(latest_snapshot.features) if latest_snapshot.features else {}
                )
                features["fetch_count"] = features.get("fetch_count", 0) + 1
                latest_snapshot.features = features
                db.commit()
                print(
                    f"   📈 {pool_id}: fetch_count incremented to {features['fetch_count']}"
                )
        except Exception as e:
    ...
    def regenerate_synthetic_pools(self):
        ...
        try:
            for pool_id, _, risk_profile, _ in synthetic_configs:
                if risk_profile == "late_crash_evolving":
                    ...
                    print(
                        f"   📈 {pool_id}: fetch count advancing to {fetch_counts[pool_id]}"
                    )
        finally:
            db.close()
        ...
        for (
            ...
            num_samples,
            risk_profile,
            force_current_risk_state,
        ) in synthetic_configs:
            # Pass the pre-computed fetch count for evolving pools
            fetch_count_override = fetch_counts.get(pool_id, None)
            self.generate_predictive_synthetic_data(
                pool_id,
                num_samples,
                risk_profile,
                force_current_risk_state,
                fetch_count_override,
            )
        ...
        try:
            ...
            if risk_profile == "late_crash":
                ...
                for c in range(num_late_crashes):
                    crash_hour = (
                        test_start + (c + 1) * crash_spacing + random.randint(-10, 10)
                    )
                    crash_hour = max(
                        test_start, min(crash_hour, num_samples - 30)
                    )  # Keep within bounds
                    late_crash_times.append(crash_hour)
                print(
                    f"   Late crash profile: scheduled crashes at hours {late_crash_times}"
                )
            ...
            if risk_profile == "late_crash_evolving":
                ...
                if fetch_count == 0:
                    print(
                        f"   ✅ Initial state: {pool_id} will show as NORMAL (LOW risk)"
                    )
                ...
                # Fetch 1: Some risk but not harmful yet - minor volatility
                    ...
                    print(
                        f"   ⚠️ First refetch: {pool_id} showing early warning signs (LOW-MEDIUM risk)"
                    )
                    ...
                    for c in range(random.randint(1, 2)):
                        crash_hour = test_start + random.randint(
                            0, num_samples - test_start - 20
                        )
                        late_crash_times.append(crash_hour)
            ...
            for i in range(num_samples):
                ...
                if (
                    risk_profile == "late_crash"
                    and i in late_crash_times
                    and regime == "normal"
                ):
                    regime = "pre_crash"
                ...
                if (
                    risk_profile == "late_crash_evolving"
                    and i in late_crash_times
                    and regime == "normal"
                ):
                    regime = "pre_crash"
                ...
                # For critical profile: Show CONTINUOUS DECLINE in RECENT data to predict HIGH risk
                # The model looks at last 48 hours, so we need strong decline in that window
                if risk_profile == "critical":
                    if i < critical_decline_start:
                        # Early period: stable/slight random walk (not declining yet)
                        regime = "normal"
                        ...
                        reserve_imbalance = random.uniform(0.02, 0.06)
                        volatility = random.uniform(0.01, 0.02)
                    else:
                        # Last ~100 hours: Accelerating decline - this is what model sees
                        hours_into_decline = i - critical_decline_start
                        decline_progress = (
                            hours_into_decline / 100
                        )  # 0 to 1 over decline period

                        if decline_progress < 0.5:
                            # First half of decline: pre_crash phase
                            regime = "pre_crash"
                            # Start with 2% decline, accelerate to 4% per hour
                            base_decline = 0.02 + (decline_progress * 0.04)
                            decline_rate = random.uniform(
                                base_decline * 0.8, base_decline * 1.2
                            )
                            noise = np.random.normal(0, 0.01)
                            current_tvl *= 1 - decline_rate + noise
                            volume_multiplier = random.uniform(2.0, 4.0)
                            reserve_imbalance = random.uniform(0.10, 0.25)
                            volatility = random.uniform(0.06, 0.10)
                        else:
                            # Last half: full crash with very high decline
                            regime = "crash"
                            # 5-10% decline per hour
                            decline_rate = random.uniform(0.05, 0.10)
                            noise = np.random.normal(0, 0.015)
                            current_tvl *= 1 - decline_rate + noise
                            volume_multiplier = random.uniform(5.0, 10.0)
                            reserve_imbalance = random.uniform(0.30, 0.50)
                    ...
                    if forced_crash_start is not None and i >= forced_crash_start:
                        if regime == "normal":
                            ...
                            regime_duration = (
                                num_samples - i + 10
                            )  # Will never reach 0 before data ends
                    ...
                    if regime == "normal":
                        ...
                        if random.random() < crash_probability:
                            regime = "pre_crash"
                            regime_duration = random.randint(
                                6, 24
                            )  # 6-24 hours warning

                    elif regime == "pre_crash":
                        # Gradual decline with increasing volatility
                        decline_rate = 0.008 + 0.005 * (
                            1 - min(regime_duration, 24) / 24
                        ...
                        if regime_duration <= 0 and (
                            forced_crash_end_protection is None
                            or i < forced_crash_end_protection
                        ):
                            regime = "crash"
                        ...
                        # Sharp decline
                        crash_rate = random.uniform(
                            0.04, 0.10
                        )  # 4-10% per hour (more severe)
                        noise = np.random.normal(0, 0.015)
                ...
                reserve0 = total_reserve * (
                    0.5 + imbalance_direction * reserve_imbalance / 2
                )
                ...
                features = {
                    ...
                    "reserve_imbalance": abs(reserve0 - reserve1)
                    / max(total_reserve, 1),
                ...
                if (
                    risk_profile == "late_crash_evolving"
                    and fetch_count_override is not None
                ):
            ...
            print(
                f"✓ Generated {num_samples} predictive synthetic snapshots for {pool_id}"
            )
            print(f"  Risk profile: {risk_profile}")
            print(f"  Crash events: {crash_counter}")
            print(f"  Final regime: {final_regime}")
            print(
                f"  TVL range: ${min(s.tvl for s in snapshots):,.0f} - ${max(s.tvl for s in snapshots):,.0f}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--all-protocols", action="store_true", help="Fetch all protocols"
    )
    parser.add_argument(
        "--synthetic", action="store_true", help="Generate synthetic data"
    )
    parser.add_argument(
        "--predictive", action="store_true", help="Generate predictive synthetic data"
    )
    parser.add_argument(
        "--pool",
    ...
    if args.all_protocols:
        ...
        fetcher.generate_predictive_synthetic_data(
            "high_risk_pool", 720, "crash_prone", force_current_risk_state=True
        )
        # Critical risk pool - uses 'critical' profile (always in crash state)
        fetcher.generate_predictive_synthetic_data(
            "critical_risk_pool", 720, "critical", force_current_risk_state=True
        )
        ...
        fetcher.generate_predictive_synthetic_data(
            "late_crash_pool_1", 720, "late_crash_evolving"
        )
        fetcher.generate_predictive_synthetic_data(
            "late_crash_pool_2", 720, "late_crash_evolving"
        )
        fetcher.generate_predictive_synthetic_data(
            "late_crash_pool_3", 720, "late_crash_evolving"
        )
        ...
        print(
            "  Note: late_crash_pool_* start normal, become harmful when data is refetched"
        )
    elif args.synthetic:
