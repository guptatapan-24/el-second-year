...
class DataFetcher:
    ...
    def generate_predictive_synthetic_data(self, pool_id: str, num_samples: int = 720, 
        ...
        try:
            ...
            # For critical profile: create data that shows ONGOING crash throughout
            # The ENTIRE time series should be in crash/pre_crash state to ensure HIGH risk
            if risk_profile == 'critical':
                regime = 'crash'  # START in crash state immediately
                regime_duration = num_samples  # Stay in crash forever
                crash_counter = 1
                # Start crash much earlier - from 50% of time series
                critical_crash_start_hour = int(num_samples * 0.3)  # Start crash at 30% mark
                current_tvl = base_tvl * 0.5  # Start at 50% of base (already declining)
            ...
            for i in range(num_samples):
                ...
                if risk_profile == 'critical':
                    if i < critical_crash_start_hour:
                        # Early period - pre-crash with warning signs
                        regime = 'pre_crash'
                        decline_rate = random.uniform(0.005, 0.015)  # 0.5-1.5% per hour
                    ...
                    elif i < critical_crash_start_hour + int(num_samples * 0.2):
                        # Middle period - accelerating decline
                        regime = 'crash'
                        decline_rate = random.uniform(0.02, 0.04)  # 2-4% per hour
                        ...
                        # Full crash phase - severe and continuous
                        regime = 'crash'
                        crash_rate = random.uniform(0.04, 0.10)  # 4-10% per hour 
                        noise = np.random.normal(0, 0.02)
                        current_tvl *= (1 - crash_rate + noise)
                        current_tvl = max(current_tvl, base_tvl * 0.02)  # 2% floor - very low
